...
type DashboardWidget = {
  id: "summary" | "composition" | "utilization" | "topBo" | "providerCost" | "workflow";
  titleId: string;
  titleEn: string;
  visible: boolean;
  roles: Account["role"][];
...
const testAccounts: Account[] = [
  { id: "u1", name: "Super Administrator", username: "superadmin", password: "Lounge@2026", role: "Super Admin", station: "ALL", scope: "Seluruh BO", organization: "Garuda Indonesia", verificationScopes: ["ALL"], status: "Aktif" },
  { id: "u2", name: "Administrator HO", username: "admin.ho", password: "Admin@2026", role: "Admin", station: "ALL", scope: "Seluruh BO", organization: "Garuda Indonesia", verificationScopes: ["ALL"], status: "Aktif" },
  { id: "u3", name: "HO Monitoring", username: "ho.monitor", password: "HO@2026", role: "HO Admin", station: "ALL", scope: "Seluruh BO", organization: "Garuda Indonesia", verificationScopes: ["REPORT"], status: "Aktif" },
  { id: "u4", name: "Branch Office CGK", username: "bo.cgk", password: "BOCGK@2026", role: "BO Admin", station: "CGK", scope: "Station CGK · Business/VIP", organization: "Branch Office CGK", verificationScopes: ["Business Class", "VIP/CIP/VVIP"], status: "Aktif" },
  { id: "u5", name: "Branch Office DJB", username: "bo.djb", password: "BODJB@2026", role: "BO Admin", station: "DJB", scope: "Station DJB · Business/VIP", organization: "Branch Office DJB", verificationScopes: ["Business Class", "VIP/CIP/VVIP"], status: "Aktif" },
  { id: "u6", name: "Petugas Lounge CGK", username: "lounge.cgk", password: "LoungeCGK@2026", role: "Lounge Officer", station: "CGK", scope: "Lounge CGK", organization: "Lounge CGK", verificationScopes: ["INPUT"], status: "Aktif" },
  { id: "u7", name: "Manager Lounge CGK", username: "manager.cgk", password: "ManagerCGK@2026", role: "Lounge Manager", station: "CGK", scope: "Exceptional Access CGK", organization: "Lounge CGK", verificationScopes: ["EXCEPTIONAL_ACCESS"], status: "Aktif" },
  { id: "u8", name: "Ancillary Verifier", username: "ancillary.verify", password: "Ancillary@2026", role: "HO Ancillary Verifier", station: "ALL", scope: "Membership/EMD/Paid Access", organization: "HO Ancillary", verificationScopes: ["Platinum", "Elite Plus", "Gold Privilege", "Elite", "GPS", "EMD", "Paid Access"], status: "Aktif" },
  { id: "u9", name: "Korean Air Verifier", username: "ke.verify", password: "Korean@2026", role: "Airline Verifier", station: "ALL", scope: "Korean Air only", organization: "Korean Air", verificationScopes: ["Korean Air", "SkyTeam"], status: "Aktif" },
];
...
export default function Home() {
  ...
  const [tab, setTab] = useState<
    ...
    >("access"),
    [lounges, setLounges] = useState<Lounge[]>(seed),
    [visitors, setVisitors] = useState<Visitor[]>([]),
    [flights, setFlights] = useState<Flight[]>(flightSeed);
  ...
  const [accounts, setAccounts] = useState<Account[]>(testAccounts),
    [partnerships, setPartnerships] = useState<Partnership[]>(partnershipSeed),
    [airlines, setAirlines] = useState<Airline[]>(airlineSeed),
    [stations, setStations] = useState<Station[]>(stationSeed),
  ...
  const [builderItems, setBuilderItems] = useState<BuilderItem[]>(builderSeed),
    ...
    [monitoringRows, setMonitoringRows] = useState<MonitoringRow[]>(monitoringSeed),
  ...
  const [showUserForm, setShowUserForm] = useState(false),
    ...
    [userDraft, setUserDraft] = useState<Omit<Account, "id">>({
      name: "", username: "", password: "", role: "BO Admin", station: "CGK", scope: "Station CGK", organization: "Branch Office CGK", verificationScopes: ["Business Class", "VIP/CIP/VVIP"], status: "Aktif",
    }),
  ...
  useEffect(() => {
    const v = localStorage.getItem("lounge-test-visitors"),
      l = localStorage.getItem("lounge-test-list"),
      f = localStorage.getItem("lounge-test-flights"),
      a = localStorage.getItem("lounge-test-accounts"),
      s = localStorage.getItem("lounge-test-stations"),
      p = localStorage.getItem("lounge-test-entitlements"),
      al = localStorage.getItem("lounge-test-airlines"),
      cc = localStorage.getItem("lounge-test-custom-columns"),
      rc = localStorage.getItem("lounge-test-report-config"),
      bi = localStorage.getItem("lounge-test-builder-items"),
      dw = localStorage.getItem("lounge-test-dashboard-widgets"),
      dr = localStorage.getItem("lounge-test-dashboard-roles"),
      mr = localStorage.getItem("lounge-test-monitoring-rows"),
      lf = localStorage.getItem("lounge-test-language-feature");
    if (v)
      setVisitors(
        (JSON.parse(v) as Visitor[]).map((x) => ({
          ...x,
          boStatus: x.boStatus || "Pending",
          boReason: x.boReason || "",
          vendorStatus: x.vendorStatus || "Pending",
          travelDate: x.travelDate || x.date,
          reconciliationStatus: x.reconciliationStatus || "Open",
        })),
      );
    if (f) setFlights(JSON.parse(f));
    if (l) {
      const saved: Lounge[] = JSON.parse(l);
      setLounges([
        ...saved,
        ...seed.filter((s) => !saved.some((x) => x.id === s.id)),
      ]);
    }
    if (a) setAccounts(JSON.parse(a));
    if (s) setStations(JSON.parse(s));
    if (p) setPartnerships(JSON.parse(p));
    if (al) setAirlines(JSON.parse(al));
    if (cc) setCustomColumns(JSON.parse(cc));
    if (rc) setReportConfig((current) => ({ ...current, ...JSON.parse(rc) }));
    if (bi) setBuilderItems(JSON.parse(bi));
    if (dw) setDashboardWidgets(JSON.parse(dw));
    if (dr) setDashboardAllowedRoles(JSON.parse(dr));
    if (mr) setMonitoringRows(JSON.parse(mr));
    if (lf != null) setLanguageFeatureEnabled(JSON.parse(lf));
  }, []);
  ...
  function login(e: FormEvent) {
    ...
    const account = accounts.find(
      (x) =>
        x.username.toLowerCase() === loginUser.trim().toLowerCase() &&
    ...
    setLoginError("");
    setLoginPassword("");
    setBoSigner(account.role === "BO Admin" ? account.name : "");
    setVendorSigner(account.role === "Lounge Officer" ? account.name : "");
    if (dashboardAllowedRoles.includes(account.role)) setTab("dashboard");
    else if (["HO Ancillary Verifier", "Airline Verifier"].includes(account.role)) { setTab("reconciliation"); setReconTab("Verifier Review"); }
    else setTab("access");
  }
  ...
  useEffect(
    () =>
      localStorage.setItem("lounge-test-visitors", JSON.stringify(visitors)),
    [visitors],
  );
  useEffect(
    () => localStorage.setItem("lounge-test-list", JSON.stringify(lounges)),
    [lounges],
  );
  useEffect(
    () => localStorage.setItem("lounge-test-flights", JSON.stringify(flights)),
    [flights],
  );
  useEffect(() => localStorage.setItem("lounge-test-accounts", JSON.stringify(accounts)), [accounts]);
  useEffect(() => localStorage.setItem("lounge-test-stations", JSON.stringify(stations)), [stations]);
  useEffect(() => localStorage.setItem("lounge-test-entitlements", JSON.stringify(partnerships)), [partnerships]);
  useEffect(() => localStorage.setItem("lounge-test-airlines", JSON.stringify(airlines)), [airlines]);
  useEffect(() => localStorage.setItem("lounge-test-custom-columns", JSON.stringify(customColumns)), [customColumns]);
  useEffect(() => localStorage.setItem("lounge-test-report-config", JSON.stringify(reportConfig)), [reportConfig]);
  useEffect(() => localStorage.setItem("lounge-test-builder-items", JSON.stringify(builderItems)), [builderItems]);
  useEffect(() => localStorage.setItem("lounge-test-dashboard-widgets", JSON.stringify(dashboardWidgets)), [dashboardWidgets]);
  useEffect(() => localStorage.setItem("lounge-test-dashboard-roles", JSON.stringify(dashboardAllowedRoles)), [dashboardAllowedRoles]);
  useEffect(() => localStorage.setItem("lounge-test-monitoring-rows", JSON.stringify(monitoringRows)), [monitoringRows]);
  useEffect(() => localStorage.setItem("lounge-test-language-feature", JSON.stringify(languageFeatureEnabled)), [languageFeatureEnabled]);
  useEffect(() => {
    if (!scanner) return;
  ...
  function save(e: FormEvent) {
    ...
    if (
      ...
      setNotice({
        kind: "error",
        text: "Data duplikat: Passenger + Flight + Check-in Sequence + Date of Travel sudah tercatat.",
      });
      return;
    }
    const n = new Date();
    ...
    setNotice({
      kind: "ok",
      text: "Akses dikonfirmasi dan otomatis masuk ke Lounge/Tenant Visitor.",
    });
  }
  ...
  function mergeFlights(incoming: Flight[]) {
    setFlights((current) => {
      const next = [...current];
  ...
  async function pdf() {
    ...
    const metadataValues: Record<string, string> = {
      ...
      "Generated Date & Local Time": `${reportDate} · ${localClock(activeTimeZone)} ${activeUtcLabel}`,
    };
    doc.setFont("helvetica", "bold");
    doc.setFontSize(14);
  ...
  function saveLounge(e: FormEvent) {
    e.preventDefault();
    ...
    if (editingLounge)
      ...
      setLounges((xs) => [...xs, { ...loungeDraft, id: crypto.randomUUID() }]);
  ...
  function saveUser(e: FormEvent) {
    e.preventDefault();
    if (!userDraft.name || !userDraft.username || !userDraft.password) return;
    ...
    if (editingUser)
      setAccounts((xs) => xs.map((x) => x.id === editingUser ? { ...normalizedDraft, id: x.id } : x));
    else setAccounts((xs) => [...xs, { ...normalizedDraft, id: crypto.randomUUID() }]);
  ...
  async function uploadUsers(file: File) {
    try {
      ...
      rows.forEach((r, index) => {
        const name = String(r["Full Name"] || "").trim(), username = String(r.Username || "").trim(),
        ...
        if (!name || !username || !validRoles.includes(accountRole) || (stationCode !== "ALL" && !stations.some((s) => s.code === stationCode)) || accounts.some((a) => a.username.toLowerCase() === username.toLowerCase()) || incoming.some((a) => a.username.toLowerCase() === username.toLowerCase())) {
          errors.push(`Baris ${index + 2}: nama, username, role, station, atau duplikasi tidak valid.`);
          return;
        }
        incoming.push({ id: crypto.randomUUID(), name, username, role: accountRole, station: stationCode, scope: stationCode === "ALL" ? "Seluruh Station" : `Station ${stationCode}`, organization: String(r.Organization || "Garuda Indonesia"), verificationScopes: String(r["Verification Scope"] || "").split(",").map((x) => x.trim()).filter(Boolean), status, password: `Temp@${Math.random().toString(36).slice(2, 8)}1` });
      });
      if (incoming.length) setAccounts((xs) => [...xs, ...incoming]);
      setUserUploadNotice(`${incoming.length} akun siap digunakan.${errors.length ? ` ${errors.length} baris gagal divalidasi.` : ""}`);
    } catch {
  ...
  function savePartnership(e: FormEvent) {
    ...
    else setPartnerships((xs) => [...xs, { ...partnershipDraft, id: crypto.randomUUID() }]);
    ...
    <main>
      <header className="top">
        <div className="brand">
          ...
          <div>
            <b>GARUDA LOUNGE ACCESS</b>
            <small>Garuda Indonesia Lounge Access Management</small>
          </div>
        </div>
          ...
          <div className="profileMenuWrap">
            ...
            {showProfileMenu && (
              <div className="profileMenu">
                ...
                <button type="button" onClick={() => { setShowInbox(true); setShowProfileMenu(false); }}>{tr("Notifikasi", "Notifications")} <strong>{visitors.filter((v) => v.boStatus !== "Accepted").length}</strong></button>
      ...
      <div className="shell">
        <aside>
          ...
          <nav>
            ...
            >
              <i>{canSeeDashboard ? "02" : "01"}</i>Lounge/Tenant Access
            </button>
            <button
              className={tab === "reconciliation" ? "active" : ""}
              onClick={() => setTab("reconciliation")}
            >
              <i>{canSeeDashboard ? "03" : "02"}</i>Visitor &amp; Reconciliation
            </button>
            {role !== "Lounge Officer" && (
              <button
              ...
              >
                <i>{canSeeDashboard ? "04" : "03"}</i>Flight Information
              </button>
            )}
            {isGlobalAdmin && (
              <button
              ...
              >
                <i>{canSeeDashboard ? "05" : "04"}</i>Master Data
              </button>
            )}
          </nav>
        </aside>
        <section className="content">
          {tab === "dashboard" && canSeeDashboard && (
            <>
              ...
              <p className="dashboardFootnote">Dashboard ini menggunakan sample monitoring data. Integrasi produksi Flight Schedule, Passenger List/DCS, membership, payment, dan evidence tetap memerlukan konfigurasi API/database oleh Tim IT.</p>
            </>
          )}
                      ...
                      <div>
                        ...
                        {canManageMaster && (
                          <div className="rowAct">
                            ...
                            <button
                              className="del"
                              onClick={() => askDelete("Hapus lounge/tenant?", `${l.airport} · ${l.name}`, () => setLounges((xs) => xs.filter((x) => x.id !== l.id)))}
                            >
                              Hapus
                            </button>
                          </div>
            ...
            <article className="card tableCard">
              <div className="actionTitle miniHead">
                ...
                {canManageMaster && <div className="rowAct"><button onClick={() => setMasterTableTarget("User & Role")}>Manage Table</button>{role === "Super Admin" && <button onClick={() => setShowManageRole(true)}>Manage Role</button>}<button onClick={downloadUserTemplate}>Template CSV</button><label className="uploadButton">Upload CSV<input type="file" accept=".csv,.xlsx,.xls" onChange={(e) => { const f = e.target.files?.[0]; if (f) void uploadUsers(f); e.target.value = ""; }} /></label><button className="primary" onClick={() => {
                  setEditingUser(null);
                  setUserDraft({ name: "", username: "", password: "", role: "BO Admin", station: "CGK", scope: "Station CGK", organization: "Branch Office CGK", verificationScopes: ["Business Class", "VIP/CIP/VVIP"], status: "Aktif" });
                        ...
                        <td>
                          {canManageMaster && a.id !== currentAccount.id ? (
                            <div className="rowAct">
                              <button onClick={() => {
                                setEditingUser(a.id);
                                setUserDraft({ name: a.name, username: a.username, password: a.password, role: a.role, station: a.station, scope: a.scope, organization: a.organization || "Garuda Indonesia", verificationScopes: a.verificationScopes || [], status: a.status });
                                setShowUserForm(true);
                              }}>Update</button>
                              <button className="del" onClick={() => askDelete("Hapus akun?", `${a.name} · ${a.username}`, () => setAccounts((xs) => xs.filter((x) => x.id !== a.id)))}>Hapus</button>
                            </div>
        ...
        <div className="back" onMouseDown={() => setShowInbox(false)}>
          <section className="modal inboxModal" onMouseDown={(e) => e.stopPropagation()}>
            ...
            <div className="notificationList">
              {verificationQueue.filter((v) => v.boStatus === "Pending").map((v) => <button key={`verify-${v.id}`} onClick={() => { setReconTab("Verifier Review"); setTab("reconciliation"); setShowInbox(false); }}><i>VERIFY</i><b>{v.name} · {v.flight}</b><span>{v.category} menunggu verifikasi {v.verifier}</span></button>)}
              {shown.filter((v) => v.boStatus === "Rejected" && v.reconciliationStatus !== "Final").map((v) => <button key={`dispute-${v.id}`} onClick={() => { setReconTab("Dispute & Correction"); setTab("reconciliation"); setShowInbox(false); }}><i>DISPUTE</i><b>{v.name} · {v.disputeCode || "Koreksi"}</b><span>{v.boReason}</span></button>)}
              {!verificationQueue.some((v) => v.boStatus === "Pending") && !shown.some((v) => v.boStatus === "Rejected" && v.reconciliationStatus !== "Final") && <div className="empty">Tidak ada aktivitas baru.</div>}
            </div>
          </section>
        </div>
      )}
      {showProfile && (
        <div className="back" onMouseDown={() => setShowProfile(false)}>
          <form className="modal" onMouseDown={(e) => e.stopPropagation()} onSubmit={(e) => {
            e.preventDefault();
            if (oldPassword !== currentAccount.password) { setProfileNotice("Password lama tidak sesuai."); return; }
            ...
            const updated = { ...currentAccount, password: newPasswordValue };
      ...
      {evidenceVisitor && (
        <div className="back" onMouseDown={() => setEvidenceVisitor(null)}>
          <form className="modal" onMouseDown={(e) => e.stopPropagation()} onSubmit={(e) => {
            ...
            setVisitors((xs) => xs.map((x) => x.id === evidenceVisitor.id ? { ...x, evidenceName, evidenceType, evidenceUploadedAt: new Date().toISOString(), boStatus: "Pending", vendorStatus: "Pending", reconciliationStatus: "Open" } : x));
            setEvidenceVisitor(null); setEvidenceName(""); setReconTab("Verifier Review");
          }}>
            ...
            <label className="evidenceUpload">Lampiran evidence<input type="file" accept=".pdf,.png,.jpg,.jpeg" onChange={(e) => setEvidenceName(e.target.files?.[0]?.name || "")} /><span>{evidenceName || "PDF/JPG/PNG, pilih satu dokumen"}</span></label>
      ...
      {deleteRequest && (
        <div className="back" onMouseDown={() => setDeleteRequest(null)}>
          <section className="modal confirmModal" onMouseDown={(e) => e.stopPropagation()}>
            <div className="confirmIcon">!</div><h2>{deleteRequest.title}</h2><p>{deleteRequest.message}</p><small>Tindakan ini tidak dapat dibatalkan pada data lokal.</small>
            <div className="modalActions"><button onClick={() => setDeleteRequest(null)}>Batal</button><button className="danger" onClick={() => { deleteRequest.action(); setDeleteRequest(null); }}>Ya, Hapus Data</button></div>
          </section>
        </div>
      )}
        ...
        <div className="back" onMouseDown={() => setShowUserForm(false)}>
          <form className="modal" onMouseDown={(e) => e.stopPropagation()} onSubmit={saveUser}>
            ...
            <div className="form">
              ...
              <label>{editingUser && role === "Super Admin" ? "Override / Temporary Password" : "Temporary Password"}<input type="password" value={userDraft.password} onChange={(e) => setUserDraft({ ...userDraft, password: e.target.value })} /></label>
