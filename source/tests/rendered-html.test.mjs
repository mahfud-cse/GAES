...
test("renders the application sign-in page", async () => {
  ...
  assert.match(html, /<title>Garuda Indonesia Lounge Access Management<\/title>/i);
  assert.match(html, /Garuda Lounge Access/i);
  assert.match(html, /garuda-wing\.svg/i);
  assert.doesNotMatch(html, /akun pengujian|data lokal|prototype/i);
});

test("keeps the v0.20 dashboard drill-down and operational controls", async () => {
