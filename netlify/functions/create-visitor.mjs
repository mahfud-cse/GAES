...
export async function handler(event) {
  try {
    ...
    const visitor = JSON.parse(event.body || "{}").visitor || {};
    const identity = [visitor.name, visitor.flight, visitor.seq, visitor.travelDate].map(normalize);
    if (identity.some((value) => !value)) return json(400, { error: "Nama, flight, sequence, dan Date of Travel wajib diisi." });
    if (visitor.eligible !== "Y") return json(422, { code: "INELIGIBLE", error: "Penumpang tidak eligible. Silakan coba scan ulang." });
    ...
    await db.runTransaction(async (transaction) => {
      if ((await transaction.get(duplicateRef)).exists) {
        throw Object.assign(new Error("Penumpang sudah ditambahkan sebagai pengguna layanan lounge."), { status: 409, code: "DUPLICATE" });
      }
      transaction.create(duplicateRef, { visitorId: visitorRef.id, identity, createdAt: new Date() });
      transaction.create(visitorRef, {
        ...visitor, id: visitorRef.id, duplicateKey, createdBy: actor.decoded.uid,
        createdAt: new Date(), updatedAt: new Date(),
      });
      transaction.create(db.collection("auditLogs").doc(), { action: "CREATE_VISITOR", targetId: visitorRef.id, actorId: actor.decoded.uid, createdAt: new Date() });
    });
